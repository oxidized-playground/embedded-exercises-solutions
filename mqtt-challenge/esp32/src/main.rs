...
use crate::writers::mqtt_writer::MqttWriter;
...
const SSID: &str = "";
const PASSWORD: &str = "";
const MQTT_ADDRESS: [u8; 4] = [192,168,1,1];
    ...
    let mut adc2 = ADC::<ADC2>::adc(analog.adc2, adc2_config).unwrap();
{
    ...
    executor.run(|spawner| {
        ...
        spawner.spawn(task(&stack, reader)).ok();
    });
}
...
async fn task(stack: &'static Stack<WifiDevice<'static>>, mut reader: adc_reader::AdcReader) {
    ...
    loop {
        ...
        let address = embassy_net::Ipv4Address::from_bytes(&MQTT_ADDRESS);
        ...
        loop {
            ...
            let adc_value: u16 = reader.read();

            // Convert temperature into String
            let mut adc_string: String<32> = String::new();
            write!(adc_string, "{}", adc_value).expect("write! failed!");


            let result = mqtt_writer.send(
                "temperature/1",
                adc_string
            ).await;
