...
fn test_mqtt_client(sensor_reading: u16) -> Result<EspMqttClient<'static>> {
    ...
    client.publish(
        ...
        QoS::AtMostOnce,
        false,
        create_creative_message(sensor_reading).as_bytes(),
    )?;
...
fn create_creative_message(sensor_reading: u16) -> String {
    format!("Do we have some emoji? {}", sensor_reading)
}
